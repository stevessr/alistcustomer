...
export function useAlistApi() {
  ...
  async function getAlistStatus() {
    try {
      ...
      const result = await invoke<ProcessMetrics>("get_alist_status");

      if (import.meta.env.MODE === "development") {
        console.log("[DEBUG] Alist Status:", result);
      }

      statusStore.status.value = {
        running: result.running,
        pid: result.pid ? Number(result.pid) : undefined,
      };

      // 添加运行状态验证
      if (result.running && !result.pid) {
        throw new Error("Process running but missing PID");
      }

      statusStore.message.value = result.running
        ? "AList运行中"
        : "AList未运行";
  ...
  async function downloadAlist() {
    try {
      ...
      const options = {
        proxyUrl:
          statusStore.useProxy && statusStore.proxyUrl
            ? statusStore.proxyUrl
            : null,
        proxyUsername:
          statusStore.useProxy && statusStore.proxyUsername
            ? statusStore.proxyUsername
            : null,
        proxyPassword:
          statusStore.useProxy && statusStore.proxyPassword
            ? statusStore.proxyPassword
            : null,
      };
